...
class CNNTransformerModel(nn.Module):
    ...
    def adapt_inputs(
        ...
        """Normalize and compute time features from absolute hours-since-epoch.
        
        NOTE: This must match the time feature computation in train.py to ensure
        consistent behavior during evaluation and training.
        """
        ...
        # Derive historical times from future_time (which are absolute hours-since-epoch)
        # future_time is (B, 24); we want (B, 168) historical hours ending just before first future hour
        first_future_hour = future_time[:, 0:1].float()  # (B, 1)
        # Create array of [168, 167, ..., 1, 0] hours before the first future hour
        hist_offset = torch.arange(self.history_len, 0, -1, device=device, dtype=torch.float32)  # (168,)
        hist_hours = first_future_hour - hist_offset.unsqueeze(0)  # (B, 168)
        hist_time_feats = make_time_feats(hist_hours)
        
        # Future time features use actual absolute hours (as in training)
